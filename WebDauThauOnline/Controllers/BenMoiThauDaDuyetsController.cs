...
namespace WebDauThauOnline.Controllers
{
    public class BenMoiThauDaDuyetsController : Controller
    {
        ...
        {
            var Loại_đăng_ký_EnumData = from Loại_đăng_ký e in Enum.GetValues(typeof(Loại_đăng_ký))
                                        select new
                                        {
                                            ID = (int)e,
            ...
            ViewBag.Loại_đăng_ký_EnumList = new SelectList(Loại_đăng_ký_EnumData, "ID", "Name");
            ...
            BenMoiThauSearchViewModel.BenMoiThauDaDuyetModel = db.BenMoiThauDaDuyets.Where(x => x.Trạng_thái == "Đã được phê duyệt").OrderBy(x => x.ID).ToPagedList(pageNumber, pageSize);
        ...
        public ActionResult GetList(BenMoiThauSearchViewModel BenMoiThauSearchViewModel, int? page)
        {
            ...
            var result = db.BenMoiThauDaDuyets.Where(x => x.Trạng_thái == "Đã được phê duyệt").AsEnumerable();

            if (searchModel.Loại_đăng_ký != null)
                result = result.Where(x => x.Loại_đăng_ký.Contains(searchModel.Loại_đăng_ký.ToDescriptionString()));
        ...
        public ActionResult Create(BenMoiThauDaDuyet benMoiThauDaDuyet)
        {
            ...
            benMoiThauDaDuyet.Loại_đăng_ký = benMoiThauDaDuyet.Loại_đăng_ký_EnumValue.ToDescriptionString();
            ...
            benMoiThauDaDuyet.Trạng_thái = "Chưa được phê duyệt";
            benMoiThauDaDuyet.AccountID = (int)Session["ID"];

            db.BenMoiThauDaDuyets.Add(benMoiThauDaDuyet);
            db.SaveChanges();

            ViewBag.UploadStatus = "Đăng ký thành công. Vui lòng đợi phê duyệt.";
            return View();
        }
